...
Savepoint는 이전 대화 context에 의존하지 않고 새 코딩 에이전트가 현재 repo/Git 상태에서 이어갈 수 있게 `.savepoint/SAVEPOINT.md`를 생성하거나 검증하는 skill입니다.
...
- application code 수정이나 savepoint라는 이름의 기능 구현 요청일 때
