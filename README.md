...
핸드오프 아티팩트는 단일 사용 bootstrap 파일로 취급됩니다. 성공적으로 재개한 뒤에는 adopted, generated, untracked 핸드오프 아티팩트만 삭제할 수 있습니다. inspect-only 요청, 안전하지 않거나 오래된 핸드오프, 추적된 파일, 사용자가 작성한 파일, 기본 생성 디렉터리 밖의 핸드오프는 명시적 지시가 없는 한 보존해야 합니다.
...
cp -R skills/new-session-handoff .agents/skills/
```
...
cp -R skills/new-session-handoff .claude/skills/
```
...
cp -R skills/new-session-handoff "$HOME/.agents/skills/"
cp -R skills/new-session-handoff "$HOME/.claude/skills/"
```
...
- 비밀은 수정되거나 생략됩니다.
- 검증된 안전한 adopted 재개는 추적되지 않는 생성된 핸드오프 아티팩트만 삭제할 수 있습니다.
