---
name: savepoint
description: "Use only for explicit coding-agent savepoint requests: create, update, inspect, or resume verified .savepoint/SAVEPOINT.md or a lightweight transfer note. Trigger on SAVEPOINT.md, repo/Git/validation recovery state, 세이브포인트 만들어줘, 세이브포인트 읽고 이어서 해줘. Do not use for SQL SAVEPOINT, ordinary summaries, docs/AGENTS authoring, code changes, /new, /status, PTY, or session rotation."
---
...
2. For verified, inspect and record cwd, Git root, branch, short HEAD, status, diff stat, name-status, staged diff, latest commit, relevant instruction files, and relevant durable state files.
