...
9. Before reporting completion for file-artifact modes, attempt to validate the generated `HANDOFF.md` with the bundled `validate_handoff.py` when available. If validation cannot run, record the skipped reason and next validation command. For prompt-only mode, self-check the response and marker block instead of running a file validator. Never claim validation passed unless it actually ran and passed.
