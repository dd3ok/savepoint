...
- Use `$savepoint` when the user asks for Korean equivalents such as `세이브포인트 만들어줘` / `세이브포인트 로드해줘` / `세이브포인트 읽어줘` / `세이브포인트 이어서 해줘`, `/savepoint save`, `/savepoint load`, `/savepoint text`, `.savepoint/SAVEPOINT.md`, or a response-only copy-paste handoff.
